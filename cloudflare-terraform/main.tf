...
resource "cloudflare_ruleset" "podcast_content_cache_bypass" {
  zone_id = var.zone_id
  name    = "Podcast content cache bypass"
  kind    = "zone"
  phase   = "http_request_cache_settings"

  rules = [{
    ref         = "podcast_content_cache_bypass"
    description = "Send podcast content requests to Caddy for statistics logging"
